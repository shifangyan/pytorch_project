...
train_dataset = GlassesDataset("/dataset/human_attribute/glassess/train.txt",
                            transform = transforms.Compose([transforms.ToPILImage(),transforms.Resize(256),transforms.RandomCrop(224),transforms.ToTensor()]))
test_dataset = GlassesDataset("/dataset/human_attribute/glassess/test.txt",
                          transform = transforms.Compose([transforms.ToPILImage(),transforms.Resize(256),transforms.RandomCrop(224),transforms.ToTensor()]))
...
model = resnet34() #实例化类LeNet  实际上也调用了LeNet的__init__()函数
model = model.cuda()  #转移到GPU上
logger.info(model)

optimizer = torch.optim.SGD(model.parameters(),learning_rate,0.90,0.0005) #实例化一个SGD优化器 参数：需要更新的参数  学习率 momentum(默认为0) 正则化项系数(默认为0) dampening(动量抑制因子 默认为0) nesterov(是否使用Nesterov 默认为false)
loss_func = torch.nn.CrossEntropyLoss()  #softmax loss函数
start_epoch = 0
#读取断点训练参数
if use_save_parameter == True:
    logger.info("begin to load parameter")
...
for epoch in range(start_epoch,num_epoch):
    for batch_img,batch_label in train_loader:
      ...
      #  print "train_acc:",train_acc
        #反向传播
        model.zero_grad()  #因为梯度是累计的 所以先将梯度清0
        ...
        if(num_iter % test_iterval == 0):
            ...
            #    print test_batch_img.shape
                ...
                output = model(test_batch_img)  #测试前向计算
