...
from HatDataset import HatDataset
...
train_batch_size = 4  #60000/64=937.5
test_batch_size = 16
image_rate = 15  #每张图片经过预处理扩展的倍数
...
load_path = "./weight/AlexNet_12000.weight"  #参数读取路径
use_save_parameter = True  #决定是否使用保存参数
checkpoint_train = False  #决定是否进行断点训练
#test_iter = 150  #测试进行多少次 10000/64 = 156 

torch.cuda.set_device(1)
train_dataset = HatDataset("/home/dataset/human_attribute/hat/train.txt",mode="train",
                            transform = transforms.Compose([Resize(256),Flip(),RandomCrop(227,"train"),ToTensor("train")]))
test_dataset = HatDataset("/home/dataset/human_attribute/hat/test.txt", mode = "test",
                          transform = transforms.Compose([Resize(256),RandomCrop(227,"test"),ToTensor("test")]))
      ...
      #  print "train_acc:",train_acc
        ...
        if(num_iter % test_iterval == 0):
            ...
            #    print test_batch_img.shape
                ...
                output = alex_model(test_batch_img)  #测试前向计算
