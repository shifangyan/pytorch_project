...
save_path = "./weight/YoloV1"   #参数保存路径 会在后面加后缀 例如 ./weight/AlexNet_1000.weight
load_path = "./weight/YoloV1_5000.weight"  #参数读取路径
use_save_parameter = False  #决定是否使用保存参数
checkpoint_train = False  #决定是否进行断点训练
train_label_txt = ["/dataset/voc2007/train.txt","/dataset/voc2012/train.txt"]
validate_label_txt = "/dataset/voc2007/test.txt"
...
class ModelTrain(object):
    ...
    def DataInitialization(self,train_transform = None,validate_transform = None):
        ...
        self.train_loader = DataLoader(train_dataset,self.train_batch_size,shuffle = True,drop_last = True)
        self.validate_loader = DataLoader(validate_dataset,self.validate_batch_size,drop_last = True)
    def ModelInitialization(self,model_function,fine_tuning,weight_path = None,last_liner_name = "classifier_liner",
                           num_classes = 2,bn = True):
        self.model = model_function(fine_tuning,weight_path = self.load_path,num_classes = 7*7*30)
        ...
        for epoch in range(self.num_epoch):
            for batch_img,batch_label in self.train_loader:
                ...
                if(self.iter % self.iter_display == 0 or self.iter == 1):
                    self.DisplayStep()
                if(self.is_validate and self.iter % self.validate_iterval == 0):
                    self.ValidateStep()
    ...
    def TrainStep(self,batch_img,batch_label):
        self.iter += 1
        #if self.epoch == 10:
        #    self.loss_func.noobj = 0.05
        #    for param_group in self.optimizer.param_groups:
        #        param_group["lr"] = 0.01
        #if self.epoch == 20:
        #    self.loss_func.noobj = 0.5
        #if self.epoch == 80:
        #    for param_group in self.optimizer.param_groups:
    ...
    def DisplayStep(self):
        self.logger.info("epoch:%d iter:%d",self.epoch,self.iter) 
        if self.iter == 1:
            iter_num = 1
        else:
...
if __name__ == "__main__":
    ...
    model_train.DataInitialization(train_transform = True,validate_transform = True)
    model_train.ModelInitialization(model_function,fine_tuning=True,weight_path = None,last_liner_name = "wearing_glasses",num_classes = 7*7*30,bn = True)
